...
/**
 * Función para validar una fecha y hora en formato YYYY-mm-dd HH:ii:ss.
 * 
 * @author Oscar Pascual Ferrero.
 * @version 1.0 Creada la función.
 * @since 2023-11-09
 * @param string $fechaHora Fecha y hora que se va a comprobar.
 * @param boolean $obligatorio Valor booleano indicado mediante 1 si es obligatorio o 0 si no lo es.
 * @return null|string Devuelve null si es correcto o un mensaje de error en caso de que lo haya.
 */
public static function validarFechaHora($fechaHora, $obligatorio = 0) {
$mensajeError = null;
$patron = "/^(\d{4})-(0[1-9]|1[0-2])-(0[1-9]|[12][0-9]|3[01]) (0[0-9]|1[0-9]|2[0-3]):([0-5][0-9]):([0-5][0-9])$/";

if ($obligatorio == 1) {
$mensajeError = self::comprobarNoVacio($fechaHora);
}

if (!preg_match($patron, $fechaHora) &&!empty($fechaHora)) {
$mensajeError .= " La fecha y hora deben estar en formato YYYY-mm-dd HH:ii:ss y ser válidas.";
}
